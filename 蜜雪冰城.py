...
def wait(sleepTime):
    ...
    while True:
        ...
        if time.time() + sleepTime - timeStamp > 0:
            print(f"当前时间{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}, 提前结束{sleepTime}s")
            break
        else:
            if abs(reduce_time) > 2:  # 如果剩余时间大于2s，则睡眠剩余的一半时间
                print(f"当前时间{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}，睡眠{abs(reduce_time) / 2}s")
                time.sleep(abs(reduce_time) / 2)

class MXMS:
    def __init__(self, atoken, proxy_url=None):
        self.headers = {
    ...
    def info(self):
        ...
        p = self.mkpayload(p)
        r = requests.get(u, headers=self.headers, params=p, proxies=self.proxies)
    ...
    def confirm(self):
        try:
            ...
            p = self.mkpayload(p)
            r = requests.post(u, headers=self.headers, json=p, proxies=self.proxies, timeout=0.75)
            print(r.text)
            if '已达领取上限' in r.text:
                return True
